...
- переходы «Монеты», «Только цена», ввод выражения и закрытие прямо из результата и графика;
- восстановление точного запроса из подписанной карточки после перезапуска, без хранения истории в базе;

Карточки версии 2.1 сохраняют работоспособность кнопок при перезапуске с тем же токеном бота. Смена токена инвалидирует прежние подписи; открыть новый запрос можно через «Монеты». Старые карточки без подписи при утрате RAM-сессии предлагают меню, не восстанавливая количество по округлённым суммам. Удаление карточки зависит от прав Telegram: при отказе бот пытается снять кнопки, массово история не очищается.
